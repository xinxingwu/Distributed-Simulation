# coding:utf-8
# This program is used to do experiment on risks/errors change with the size of samples
...
tf.app.flags.DEFINE_float('lamb', 0.0001, 'The regularization parameter')
tf.app.flags.DEFINE_integer('device_num', 4, 'Number of devices')

# Here, it sets the maximal amount of working computers be 12. However, it can be changed. But considering the uniformity
# in distributed setting, it is better to set a maximal amount of working computers before computing
...
# Assigning the samples to working computers in distributing setting
slice_data = []
...
for device_index in range(device_num):
    ...
    with tf.device(device):
        ...
        with tf.Session() as sess:
            ...
            # Here, Inv_On_Trans_X_dot_X_Identity is like (Trans_X_dot_X+lamb times Identity_I)^{-1}
...
with tf.Session(server.target) as sess:
    ...
    fo = open("../Data/SampleIncrease.txt", "a+")
