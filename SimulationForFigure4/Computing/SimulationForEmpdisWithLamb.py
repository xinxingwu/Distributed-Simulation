#coding:utf-8
#SimulationForEmpdisWithLamb.py, using the code to train the model, and obtain the expected risk, empirical risk and so on
...
tf.app.flags.DEFINE_integer('training_epochs', 1000,'Steps to train and print loss')
tf.app.flags.DEFINE_integer('display_step', 200,'Steps to disply the process of training')
tf.app.flags.DEFINE_integer('device_num', 4,'Number of devices')#32
...
tf.app.flags.DEFINE_float('lamb', 0.60,'The regularization parameter')
...
with tf.Session(server.target) as sess:
    ...
    emp_exp=(w_for_average_model[0]-2)*(w_for_average_model[0]-2)*(4.0/3)+2*(w_for_average_model[0]-2)*(b_for_average_model-2)+(w_for_average_model[1]+1)*(w_for_average_model[1]+1)+(b_for_average_model-2)*(b_for_average_model-2)+1.5
