#coding:utf-8
#SimulationForEmpdis.py, this program is used to train the model. Furthermore, we also use this program to obtain weights and bias and so on
...
tf.app.flags.DEFINE_integer('device_num', 20,'Number of devices')
...
#num=9,19,29
#loss = tf.multiply(tf.reduce_mean(tf.square(y - Y)),(batch_on_computers-1.0+batch_on_computers/(device_num-1.0))/(batch_on_computers+batch_on_computers/(device_num-1.0)))
...
for device_index in range(device_num):
    ...
    with tf.device(device):
        ...
        with tf.Session() as sess:
            ...
            # num=9,19,29
            #x_data_fetch = np.append(x_data[:, batch_on_computers * device_index:batch_on_computers * (device_index + 1)],x_data[:, batch_on_computers * device_num+2*device_index:batch_on_computers * device_num+2*(device_index+1)],axis=1)
            #y_data_fetch = np.append(y_data[batch_on_computers * device_index:batch_on_computers * (device_index + 1)],y_data[batch_on_computers * device_num+2*device_index:batch_on_computers * device_num+2*(device_index+1)])
...
with tf.Session(server.target) as sess:
    ...
    fo.write(str(w_for_average_model[0]) + ",")
    fo.write(str(w_for_average_model[1]) + ",")
    fo.write(str(b_for_average_model)+"\n")
    fo.close()
