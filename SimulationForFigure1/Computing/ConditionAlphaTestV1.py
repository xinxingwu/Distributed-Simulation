...
alpha=0
while (alpha <=1):
    reg_1 = np.sum(np.square(np.matmul(w_for_average_2, x_data) + b_data[1] - y_data)) / complexity_sample + np.multiply(lamb, np.multiply(la.norm(w_for_average_21), la.norm(w_for_average_21)))
    print reg_1
    reg_1_plus=np.sum(np.square(np.matmul((1-alpha)*w_for_average_2+alpha*w_for_average_1, x_data) + (1-alpha)*b_data[1]+alpha*b_data[0] - y_data)) / complexity_sample + np.multiply(lamb, np.multiply(la.norm((1-alpha)*w_for_average_21+alpha*w_for_average_11), la.norm((1-alpha)*w_for_average_21+alpha*w_for_average_11)))
    print reg_1_plus

    fo = open("../Data/eqv1.txt", "a+")
    fo.write(str(alpha) + ",")
